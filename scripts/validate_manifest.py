#!/usr/bin/env python3
"""Validate the core Scene Manifest contract for a vector drawing job."""
...
VECTOR_PROVIDERS = {"xiaomiao", "local-vector-service", "vtracer", "local"}
...
def audit(path: Path) -> dict[str, Any]:
    ...
    for index, obj in enumerate(objects):
        ...
        if object_type in SEMANTIC_TYPES:
            semantic_count += 1
            if provider_of(obj) not in VECTOR_PROVIDERS:
                errors.append(f"{object_id} semantic subject is not sourced from an accepted vector backend.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a vector manifest.json.")
