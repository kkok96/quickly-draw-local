...
$runtimePath = Join-Path $PSScriptRoot 'cached_vector_runtime.jsx'
...
function Invoke-CachedRuntime([object]$illustrator, [object]$configuration) {
    ...
    $bootstrap = "var VECTOR_PLAYBACK_CONFIG = $configJson; `$`.evalFile(new File($runtimeJson));"
...
function Connect-IllustratorCom {
    ...
    $emptyDocumentCandidate = $null
    foreach ($progId in $progIds) {
        try {
            $candidate = New-Object -ComObject $progId
            try {
                if ($candidate.Documents.Count -ge 1) { return $candidate }
            } catch {
                [Runtime.InteropServices.Marshal]::ReleaseComObject($candidate) | Out-Null
                continue
            }
            if ($null -eq $emptyDocumentCandidate) {
                $emptyDocumentCandidate = $candidate
            } else {
                [Runtime.InteropServices.Marshal]::ReleaseComObject($candidate) | Out-Null
            }
        }
    ...
    if ($null -ne $emptyDocumentCandidate) { return $emptyDocumentCandidate }
    throw 'AI_COM_UNAVAILABLE|No registered Illustrator COM interface was found.'
}
...
try {
    ...
    $illustrator = Connect-IllustratorCom
    ...
    Write-Output "VECTOR_PLAYBACK_COMPLETE|cache=$cachePath|ai=$OutputAi|png=$OutputPng|batches=$completedCount/$($state.batches.Count)|mode=$mode|illustrator_window_untouched=true"
} finally {
